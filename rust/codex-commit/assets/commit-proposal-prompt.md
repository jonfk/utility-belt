...
- Treat user-provided commit context as a hint about intent, not as evidence of changes.
- Use the user context to choose emphasis, wording, and likely commit scope when the repository state supports it.
- Prefer wording that reflects the user's stated focus when several accurate phrasings are possible.
- Use the user context to decide what to foreground in `summary`, `commit.subject`, and `commit.body_paragraphs`.
- Never let user context justify claims, files, or scope that are not supported by the diff.
- Example: if the user says `focus on tests` and the diff includes tests plus source changes, emphasize tests only when that matches the actual substance of the change.
...
- Use the user-provided commit context to improve wording when it fits the diff, but do not claim changes that are not present in the diff.

## Workflow
1. Read and retain any user-provided commit context.
- Treat it as advisory input for intent, emphasis, and ambiguity resolution.
- Use it to resolve ambiguity between multiple plausible commit summaries or split options.
- If the diff contradicts the user context, follow the diff and treat the user context as advisory only.
...
3. Decide what to read in detail.
