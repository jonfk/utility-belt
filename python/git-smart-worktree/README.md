...
```
  ...
  ~/worktrees/github.com/acme/payment-service/main/main
  ~/worktrees/github.com/acme/payment-service/feature/user-payments
  ~/worktrees/github.com/acme/payment-service/review/pr-842
...
- The git worktrees are stored at GIT_WORKTREE_ROOT/<host>/<owner>/<repo>/<context>/<branch-slug>.
...
- add command: I can add a git worktree for the current directory's repository. It should take a context, a branch name and an optional starting point. If those are not provided and add is called without these arguments, it should open in an interactive mode where these are prompted to the user to select or add values for them.
    - context: A folder like feature|review|fix that organizes worktrees into categories. At the interactive prompt, the user should be able to select from a predefined list or create a new one
